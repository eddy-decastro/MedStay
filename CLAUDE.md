...
conteneurisée et déployée automatiquement sur Hugging Face Spaces.
...
- Déploiement : Hugging Face Spaces UNIQUEMENT (Docker SDK, port 7860)

## Contraintes non négociables

1. **0 € de coût.** Aucun service payant, aucune carte bancaire. Pas de déploiement AWS réel :
   le dossier `infra/` contient un Terraform/Dockerfile AWS documenté mais jamais exécuté en CI.
...
4. **Front découplé** : Streamlit → HTTP → FastAPI → artefacts. Un seul Space HF, deux
   processus lancés par `start.sh` (uvicorn port 8000 interne, Streamlit port 7860 exposé).
...
├── Dockerfile
├── docker-compose.yml
├── start.sh           # lance uvicorn + streamlit dans le conteneur HF
...
docker build -t medstay . && docker run -p 7860:7860 medstay
```
...
- Space de type Docker, port exposé 7860 (obligatoire HF).
- `start.sh` : uvicorn en arrière-plan sur 8000, puis Streamlit sur 7860.
  Streamlit appelle `http://localhost:8000`.
- `deploy.yml` : sur push `main` et après CI verte, push du repo vers le Space via
  `HF_TOKEN` (secret GitHub). Ne jamais écrire le token en clair.
- Construire le CI/CD par incréments : (1) ruff seul → (2) + pytest → (3) + docker build
  → (4) + deploy. Une étape verte avant d'ajouter la suivante.

## Ordre des phases (résumé — détail dans docs/ROADMAP.md)

0. Setup repo/env → 1. Squelette déployable (API factice + Docker simple + Space créé) →
